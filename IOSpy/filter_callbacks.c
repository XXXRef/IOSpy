...
#define MAX_FILE_PATH_LENGTH 600
	...
	DbgPrint("[CREATE_FILE] FilePath: %ws PID: %X TID :%p flagFileCreated:%d FileObject:\n\0", Name, requestorPID, threadID, flagFileCreated);
	// Save data in log file
#define MAX_LOG_RECORD_LENGTH 1000
	WCHAR logRecord[MAX_LOG_RECORD_LENGTH];
	swprintf(logRecord, L"[CREATE_FILE] FilePath:%ws PID:%X TID:%p flagFileCreated:%d FileObject:\n\0", Name, requestorPID, threadID, flagFileCreated);
